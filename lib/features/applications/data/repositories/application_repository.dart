...
class ApplicationRepository {
  ...
  Future<void> submitApplication({
    ...
    if (alreadyApplied) {
      throw StateError('You have already applied for this opportunity.');
    }

    final applicationReference = _applications.doc();
    ...
    batch.set(notificationReference, {
      ...
      'message': '${student.fullName} applied for ${opportunity.title}.',
  ...
  Future<void> updateApplicationStatus({
    ...
    final applicationReference = _applications.doc(applicationId);

    final snapshot = await applicationReference.get();

    final data = snapshot.data();

    if (!snapshot.exists || data == null) {
      throw StateError('The application could not be found.');
    }

    final previousStatus = data['status'] as String? ?? 'submitted';
    ...
    final opportunityTitle =
        data['opportunityTitle'] as String? ?? 'the opportunity';

    final startupName = data['startupName'] as String? ?? 'The startup';
    ...
    if (studentId.isNotEmpty && actorId.isNotEmpty) {
      final notificationReference = _notifications.doc();
  ...
  Future<void> scheduleInterview({
    ...
    final startupOwnerId = data['startupOwnerId'] as String? ?? '';

    final actorId = _firebaseAuth.currentUser?.uid ?? startupOwnerId;

    if (studentId.isEmpty) {
      throw StateError('The student account could not be found.');
    }
    ...
    batch.update(applicationReference, {
      ...
      'interviewLocationOrLink': locationOrLink.trim(),
      'interviewNotes': notes.trim(),
      'interviewScheduledBy': actorId,
      'interviewScheduledAt': FieldValue.serverTimestamp(),
      'updatedAt': FieldValue.serverTimestamp(),
    });
  ...
  Future<void> withdrawApplication(String applicationId) async {
    ...
    final opportunityTitle =
        data['opportunityTitle'] as String? ?? 'an opportunity';

    final actorId = _firebaseAuth.currentUser?.uid ?? studentId;
    ...
    if (startupOwnerId.isNotEmpty && actorId.isNotEmpty) {
      final notificationReference = _notifications.doc();
